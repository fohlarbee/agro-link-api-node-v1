...
import { ShiftCreationResponse, ShiftListResponse } from './entities/shift.entity';
...
export class ShiftsController {
  ...
  create(@Body() createShiftDto: CreateShiftDto, @Req() request: Record<string, any>) {
  ...
  assignTables(@Param("id") shiftId: string, @Body() { tableIds }: AssignShiftTablesDto, @Req() request: Record<string, any>) {
    const { business_id } = request.headers
    return this.shiftsService.assignShiftTables(+business_id, +shiftId, tableIds);
  }
