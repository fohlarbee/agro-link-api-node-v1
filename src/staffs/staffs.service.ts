...
export class StaffsService {
  ...
  async createStaff(restaurantId: number, { email, name, roleId }: CreateStaffDto) {
    ...
    if (user) return this.inviteExistingUser({ restaurantId, userId: user.id, roleId });
    return this.inviteNewUser({ restaurantId, email, name, roleId });
  }

  private async inviteExistingUser({ restaurantId, userId, roleId }: 
    { restaurantId: number, userId: number, roleId: number }) {
    const staff = await this.prisma
      .staff
      .findFirst({ where: { userId, restaurantId }});
  ...
  private async inviteNewUser({ restaurantId, email, name, roleId }: 
    { restaurantId: number, email: string, name: string, roleId: number }) {
    const hashedPassword = bcrypt.hashSync(process.env.NEW_ADMIN_PASSWORD, bcrypt.genSaltSync());
    const user = await this.prisma
      .user
      .create({
  ...
  async findAllStaffs(restaurantId: number) {
    const staffs = await this.prisma
      .staff
      .findMany({
        where: { restaurantId },
        select: {
